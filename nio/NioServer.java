...
public class NioServer {
    ...
    private void handleRead(SelectionKey key) throws IOException {
        ...
        while (true) {
            ...
            if (read == 2) { //В терминале Windows условие read == 0 работало некорректно, так что опытным путём решено было поставить такой костыль...
                break;
            }
            buf.flip();
        ...
        } else if (command.startsWith("mkdir")) {//-------------------------создание каталога------------------
            command = command.replaceAll("mkdir +", "").trim();
            if (Files.notExists(root.resolve(command))) {
                Files.createDirectory(root.resolve(command));
                ByteBuffer response = ByteBuffer.wrap(String.format("Directory %s was create! %n", command).getBytes());
                channel.write(response);
            } else {
                ByteBuffer response = ByteBuffer.wrap(String.format("directory with the same name already exists! %n").getBytes());
                channel.write(response);
            }//----------------------конец создания каталога---------------------------------


        } else if (command.startsWith("touch")) {//--------------------------Создание файла-----------------------
            command = command.replaceAll("touch +", "").trim();
            if (Files.notExists(root.resolve(command))) {
                Files.write(root.resolve(command), "".getBytes(StandardCharsets.UTF_8));
                ByteBuffer response = ByteBuffer.wrap(String.format("File %s was create! %n", command).getBytes());
                channel.write(response);
            } else {
                ByteBuffer response = ByteBuffer.wrap(String.format("File with the same name already exists! %n").getBytes());
                channel.write(response);
